...
class MSVDVocab(CustomVocab):
    ...
    def load_captions(self):
        ...
        df_s = pd.read_csv('dataset/EmVidCap-S/EmVideo_trainval_captions.csv')
        style_s = df_s['EmCaptions'].values

        df_l = pd.read_csv('dataset/EmVidCap-L/EmVideo_trainval_captions.csv')
    ...
    def load_captions(self):
        if len(self.caption_fpath.split('/')[-1]) < 10:
            ...
            for video_id, start, end, caption in df.values:
                ...
                self.msvd_captions[vid].append(caption)
        else:
            ...
            for video_id, caption in df_l.values:
                ...
                self.msvd_captions[video_id].append(caption)
            ...
            df_msvd = df_msvd[df_msvd['Language'] == 'English']
            df_msvd = df_msvd[[ 'VideoID', 'Start', 'End', 'Description' ]]
            df_msvd = df_msvd[pd.notnull(df_msvd['Description'])]
            for video_id, start, end, caption in df_msvd.values:
                vid = "{}_{}_{}".format(video_id, start, end)
                self.msvd_captions[vid].append(caption)

            dual_dataset = None
            if self.caption_fpath.split("/")[1]=="EmVidCap-L":
                dual_dataset = "EmVidCap-S"
            elif self.caption_fpath.split("/")[1]=="EmVidCap-S":
                dual_dataset = "EmVidCap-L"
            if dual_dataset != None:
                if self.split == 'train':
                    df_s = pd.read_csv('dataset/{}/EmVideo_trainval_captions.csv'.format(dual_dataset))   
                else:
                    df_s = pd.read_csv('dataset/{}/EmVideo_test_captions.csv'.format(dual_dataset))            
