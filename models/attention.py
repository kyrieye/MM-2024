...
class DynamicSemanticModule(nn.Module):
    def __init__(self,emo_size,feat_size,k,h_list) -> None:
        ...
        self.agg_w_a = nn.Linear(self.feat_size,self.aggregation_k,bias=False)
        ...
        self.subspace_wq = nn.Linear(self.emo_size,self.d_sum,bias=False)
        self.subspace_wk = nn.Linear(self.emo_size,self.d_sum,bias=False)
        self.subspace_wv = nn.Linear(self.emo_size,self.h_sum,bias=False)
        self.subspace_wr = nn.Linear(self.emo_size,len(self.h_list),bias=False)
    ...
    def forward(self,emo_embedding,phr_feat):
        aggre_feat = self.feature_aggregation(phr_feat)
    ...
    def feature_aggregation(self,phr_feat):
        A_agg = self.agg_w_a(phr_feat)
        feat_a = self.agg_w_c(phr_feat)
