...
class Transformer_emo(nn.Module):
    ...
    def forward(self, src_seq, trg_seq, src_seq_34,sub_mode,pos_em_word):
        ...
        if sub_mode==False:
            ...
            src_mask=torch.zeros(size=(batch_size,30,179)).cuda()
            for i in range(batch_size):
                position = (pos_em_word[i].cpu().numpy() - 1)
                pos_sub_emo = list(set(position))
                if -1 in pos_sub_emo:
                    pos_sub_emo.remove(-1)
                src_mask[i,:,pos_sub_emo] = 1
                #position = (pos_em_word[i].cpu().numpy() - 1)[0]
                #if position != -1:
                    #src_mask[i,:,position] = 1
