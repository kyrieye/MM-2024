...
class EmotionCaptioningNetwork(nn.Module):
    ...
    def forward_decoder(self, batch_size, vocab_size, pos_vis_feats, pos_captions, pos_em_word, neg_vis_feats, neg_captions,
                        teacher_forcing_ratio, batch_id,vocab,sub_mode):
        ...
        em_embeding, em_logits1, em_logits2 = self.emotion_attention(src_seq=emotion_vocab_feats, trg_seq=pos_vis_feats,src_seq_34=emotion_cate_feats,sub_mode=sub_mode,pos_em_word=pos_em_word)
        sub_emo = em_embeding
        ...
        for t in range(1, self.max_caption_len + 2):
            ...
            output, hidden,  sem_align_weights, sem_align_logits,em_embeding = self.decoder(sub_emo,em_embeding, embedded, \
                                                hidden, pos_vis_feats, phr_feats, t,emotion_vocab_feats,emotion_cate_feats,sub_mode)
    ...
    def forward(self, pos_vis_feats, pos_captions, pos_em_word, neg_vis_feats, neg_captions,batch_id, vocab,teacher_forcing_ratio=0.,sub_mode=False):
        ...
        captions, CA_logits, cls_attention1, cls_attention2 = self.forward_decoder(batch_size, vocab_size, \
            pos_vis_feats, pos_captions, pos_em_word,neg_vis_feats, neg_captions, teacher_forcing_ratio, batch_id,vocab,sub_mode=sub_mode)
        return captions, CA_logits, cls_attention1, cls_attention2

    def describe(self, vis_feats,pos_em_word,sub_mode):
        batch_size = vis_feats['clip'].size(0)
        vocab_size = self.decoder.output_size

        vis_feats = self.forward_visual_encoder(vis_feats)
        captions,words = self.beam_search(batch_size, vocab_size, vis_feats, pos_em_word,sub_mode=sub_mode)
        return captions,words


    def beam_search(self, batch_size, vocab_size, vis_feats, pos_em_word,width=5,sub_mode=False):
        emotion_vocab_feats = []
        for idx,word in enumerate(self.vocab.em_vocabs):
            word = Variable(torch.cuda.LongTensor(1).fill_(self.vocab.em_word2idx[word]))
            weights = self.embedding_emo(word)
        ...
        em_embeding, emo_dist1, emo_dist2 = self.emotion_attention(src_seq=emotion_vocab_feats, \
            trg_seq=vis_feats,src_seq_34=emotion_cate_feats,sub_mode = sub_mode, pos_em_word = pos_em_word)
        sub_emo = em_embeding
        ...
        for t in range(1, self.max_caption_len + 2):
            ...
            for i, (input, hidden, cum_prob) in enumerate(zip(input_list, hidden_list, cum_prob_list)):
                ...
                output, next_hidden,semantic_align_weights, semantic_align_logits,em_embeding= self.decoder(sub_emo,em_embeding,embedded, hidden, vis_feats, phr_feats, t,emotion_vocab_feats,emotion_cate_feats,sub_mode)
