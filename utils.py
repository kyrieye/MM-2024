...
def train(e, model, optimizer, train_iter, vocab, teacher_forcing_ratio, C, cross_wei):
    ...
    for batch in pgbar:
        ...
        output, contrastive_attention, cls_attention1, cls_attention2 = model(
            pos_vis_feats, pos_captions, pos_em_word, neg_vis_feats, neg_captions, batch_id, vocab,teacher_forcing_ratio)
        ...
        #loss = cross_entropy_loss + C.CA_lambda * contrastive_attention_loss 
        ...
        #pgbar.set_description("[Epoch #{0}] loss: {1:.3f} = CE {2:.3f} + CTR {3} * {4:.3f}".format(
            #e, loss,cross_entropy_loss,C.CA_lambda,contrastive_attention_loss ))
...
def score_full( model, data_iter, vocab):
    ...
    for vids, feats,pos_em_word in tqdm(YOLO_iter, desc='score'):
        captions,_ = model.describe(feats,pos_em_word)
