...
def train(e, model, optimizer, train_iter, vocab, teacher_forcing_ratio, C, cross_wei,sub_mode):
    ...
    for batch in pgbar:
        ...
        output, contrastive_attention, cls_attention1, cls_attention2 = model(
            pos_vis_feats, pos_captions, pos_em_word, neg_vis_feats, neg_captions, batch_id, vocab,teacher_forcing_ratio,sub_mode)
...
def build_YOLO_iter(data_iter, batch_size):
    ...
    while len(vids) > 0:
        ...
        yield ( vids_batch, feats_batch,pos_em_word_batch )
...
def score_full(model, data_iter, vocab,dataset,sub_mode,e):
    ...
    hypos = {}
    for vids, feats,pos_em_word in tqdm(YOLO_iter, desc='score'):
        captions,_ = model.describe(feats,pos_em_word,sub_mode)
    ...
    scores = calc_scores(refs, hypos,dataset,e)
    
    emotion_path = 'dataset/sentiment-words-E/'
    Acc_sw, Acc_c = get_result(hypos, emotion_path, refs,dataset)
...
def calc_scores(ref, hypo,dataset,e):
    """
    ...
    for scorer, method in scorers:
        if method == "Meteor":
            score, scores = scorer.compute_score(ref, hypo,dataset,e)
        else:
            score, scores = scorer.compute_score(ref, hypo,dataset)
        if type(score) == list:
